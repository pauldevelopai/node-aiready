...
/** Stable id: canonical URL for url/sitemap sources, 'drive:'+fileId for Drive. */
export function articleId({ source_kind, source_ref }) {
  const basis = source_kind === 'drive' ? `drive:${source_ref}` : canonicalUrl(source_ref);
...
export function makeArticle({ source_kind, source_ref, url = null, title = null }) {
  ...
  return {
    ...
    source_kind,                 // 'url' | 'sitemap' | 'drive'
    source_ref,                  // the URL, or the Drive fileId
    source_format: null,         // 'html' | 'pdf' | 'docx' | 'doc' | 'gdoc' | 'text'
    url,                         // canonical public URL (null for Drive-only)
