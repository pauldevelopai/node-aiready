...
export async function ingestUrls(host, urls, { force = false, sourceKind = 'url', onProgress } = {}) {
  ...
  await pool(clean, CONCURRENCY, async (url) => {
    try {
      ...
      const title = row.title || meta.title || res.title || null;
      await putArticle(host, {
        ...row,
        url: canonicalUrl(res.finalUrl || url),
        title,
        slug: row.title ? row.slug : slugify(title || url, row.slug),
        // Prefer any existing value (a manual edit) over re-extracted metadata.
        ...
        summary: row.summary || meta.description || null,   // pre-seed llms.txt; AI generate can refine
        source_format: 'html',
