...
export function mountAppRoutes(app, getHost) {
  ...
  // After any ingest that added/updated docs, build the semantic index for them so
  // "Ask the archive" + Search work immediately — no manual "Build semantic index"
  // click. Best-effort + non-fatal: the ingest already succeeded, and without an
  // OpenAI key this is a no-op (search falls back to keyword). Idempotent (embedAll
  // skips already-embedded docs).
  async function autoEmbed(host, stats) {
    if (!embeddingsAvailable() || ((stats?.added || 0) + (stats?.updated || 0)) <= 0) return 0;
    try { return (await embedAll(host)).embedded || 0; }
    catch (err) { console.error('aiready auto-embed failed:', err.message); return 0; }
  }
  ...
  app.post('/api/aiready/ingest/urls', wrap(async (req, host) => {
    ...
    if (!urls.length) return { ok: false, message: 'Paste one or more page URLs (one per line).' };
    const stats = await ingestUrls(host, urls, { force: !!req.body?.force });
    return { ok: true, ...stats, embedded: await autoEmbed(host, stats) };
  }));
  app.post('/api/aiready/ingest/sitemap', wrap(async (req, host) => {
    ...
    return r.error ? { ok: false, message: r.error } : { ok: true, ...r, embedded: await autoEmbed(host, r) };
  }));
  app.post('/api/aiready/ingest/drive', wrap(async (req, host) => {
    ...
    return r.error ? { ok: false, message: r.message, reason: r.error } : { ok: true, ...r, embedded: await autoEmbed(host, r) };
  }));
  ...
  app.post('/api/aiready/ingest/upload', uploadFiles, wrap(async (req, host) => {
    ...
    const stats = await ingestFiles(host, files, { force: !!(req.body && req.body.force) });
    return { ok: true, ...stats, embedded: await autoEmbed(host, stats) };
  }));
