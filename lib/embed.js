...
export async function embedAll(host, { force = false } = {}) {
  ...
  console.log(`[aiready embed] start · key present=${!!_k} len=${_k.length} prefix=${_k.slice(0, 6)}`);
  ...
  for (const a of todo) {
    try {
      ...
      console.error(`[aiready embed] ${a.id} failed: ${e.message}`);
      if (/401|403/.test(e.message)) { stats.errors.push({ fatal: 'Key rejected by OpenAI.' }); break; }
    }
  }
  if (stats.total && !stats.embedded) console.error(`[aiready embed] all ${stats.total} failed · first: ${stats.errors[0]?.status || 'unknown'}`);
