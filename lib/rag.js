...
export async function askArchive(host, question, { limit = MAX_SOURCES } = {}) {
  ...
  if (!hits.length) {
    return { ok: true, answer: "I couldn't find anything in your documents about that yet.", sources: [], mode: found.mode };
  }
  ...
  if (!sources.length) {
    return { ok: true, answer: "I found matching documents but none have converted text yet — run conversion first.", sources: [], mode: found.mode };
  }

  const ground = formatContextForPrompt(await getProfile(host));
  const system =
    'You are a research assistant answering questions using ONLY the '
    + 'organisation\'s own documents, supplied as numbered sources. Rules: (1) use ONLY '
