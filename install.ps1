# install.ps1 — one-command installer for the AI Ready Archive Node (Windows).
#
...
$Repo    = if ($env:GROUNDED_REPO)         { $env:GROUNDED_REPO }         else { 'pauldevelopai/node-aiready' }
...
$AppDir  = Join-Path $Root 'node-aiready'
$Name    = 'AI Ready Archive'
