#!/usr/bin/env bash
#
# install.sh — one-command installer for the AI Ready Archive Node (macOS).
#
...
REPO="${GROUNDED_REPO:-pauldevelopai/node-aiready}"
REF="${GROUNDED_REF:-main}"
NODE_VERSION="${GROUNDED_NODE_VERSION:-20.18.1}"
GROUNDED_HOME="${GROUNDED_HOME:-$HOME/GROUNDED}"
APP_DIR="$GROUNDED_HOME/node-aiready"
DISPLAY_NAME="AI Ready Archive"
PORT="${PORT:-3000}"
